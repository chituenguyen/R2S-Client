...
const API_ORDER_URL = "https://devapi.uniscore.vn/uri/api/orders"
...
const CheckOut = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Order placed successfully!") // Hiển thị toast thành công
        localStorage.removeItem("cart")
        setCart([])
        setTotal(0)
      } else {
        toast.error("Failed to place order") // Hiển thị toast lỗi
      }
    } catch (error) {
      console.error("Order error:", error)
      toast.error("An error occurred while placing the order") // Hiển thị toast lỗi khi gặp sự cố
    }
  }
